...
const ResponderSospecha = function (params) {
  const { ws } = params;
  const { sospecha_en_curso } = params;

  const [eleg, setEleg] = useState({ nombre: '', elegido: false });

  const { sospecha } = params;
    ...
    <div>
      {sospecha_en_curso
        ...
        <div className="fondo_respuesta">
          ...
          {eleg.elegido && (
          <button onClick={respuesta_sospecha}>
            Elegir
            {' '}
            {eleg.nombre}
          </button>
