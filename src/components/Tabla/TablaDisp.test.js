...
const server = setupServer(
  rest.get('http://localhost:8000/partidas', (req, res, ctx) => res(ctx.status(200), ctx.json(MOCK_GET))),
);
...
test('No hay partidas disponibles', async () => {
  server.use(
    rest.get('http://localhost:8000/partidas', (req, res, ctx) => res(ctx.status(200), ctx.json([]))),
  );
