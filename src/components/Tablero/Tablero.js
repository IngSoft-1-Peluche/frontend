...
const Tablero = function (params) {
  ...
  const EstadoJugadores = (id) => (
    lista.map((jugador) => (
      parseInt(jugador.posicion) === parseInt(id) && <div className="dot" key={jugador.apodo} style={{ backgroundColor: jugador.color }} />
    )));
  ...
  const Casillero = function ({
    id, imagen, width, height,
  }) {
    return (
